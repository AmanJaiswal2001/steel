...
const Sheets300 = ({type}) => {
    ...
    return (
        ...
        {filteredCards.slice(currentIndex, currentIndex + visibleCards).map((card, index) => (
         ...
         <button
             className={`absolute left-0 h-9 w-9 top-[40%] hidden bg-[#e6f0ff] lg:flex items-center justify-center bg-opacity-50 cursor-pointer text-[#2241a6] p-1 rounded-lg hover:bg-[#d7e7f16 ${currentIndex===0?"hidden":"block"}`}
             onClick={prevSlide}
