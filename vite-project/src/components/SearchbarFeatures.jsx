      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.length ? filtered.map(item => (
          ...
          <figure className='w-full object-fit '>
            ...
            <h2 className="  mt-2 font-sans font-medium h-12  text-[1rem]">{item.title}</h2>
            <div className='flex gap-2'>
            <span className='font-normal text-[12px] font-sans text-[#70737a]'>Brand:</span>
            <p className=' font-sans font-semibold text-[#70737a]  text-[12px]'> {item.brand}</p>
     
     </div>
     <div className='flex gap-2'>
      <span className='font-semibold text-sm font-sans text-[#262626]'>Thickness</span>
      <p className=' font-medium text-sm font-sans text-[#262626]'> {item.thickness}</p>
     
      </div>
      <div className='flex gap-2'>
      <span className='font-semibold text-sm font-sans text-[#262626]'>Width </span>
      <p className=' font-medium text-sm font-sans text-[#262626]'> {item.width}</p>
     
      </div>
      <div className='flex gap-2'>
      <span className='font-semibold text-sm font-sans text-[#262626]'>Length</span>
      <p className=' font-medium text-sm font-sans text-[#262626]'> {item.length}</p>
     
      </div>
      <div className='bg-[#f5f6fa]  h-20 rounded-b-lg w-full bottom-0 absolute left-0'>
      <div className="  w-52 m-0 p-0 text-left  h-10 ">
        <button className=" p-2 w-full m-0  text-[#2241a6] text-left bg-bl-200 font-sans cursor-pointer font-semibold  text-sm  ">{item.buttonName}</button>
      </div>
      ...
      <div className='flex gap-1 px-2 pt-2'>
        ...
        <span className='font-sans font-medium text-[#262626]   text-sm'>{item.delivery}  </span>
      </div>
      </div>   
      </div> </div>
        )) : (
